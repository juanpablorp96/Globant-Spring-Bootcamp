...
public class GameController {
	...
	@RequestMapping(value = "/game-player1")
	public List<Game> getGame() {
		
		final List<Game> result = new ArrayList<>();

		//player have 3 turns
		if (gameTotalPlayer1.getCount() != 3) {
			
			int randomNum = ThreadLocalRandom.current().nextInt(0, 5000 + 1);

			gameTurn.setScore(randomNum);
			gameTotalPlayer1.setScore(gameTotalPlayer1.getScore() + randomNum);			
			gameTotalPlayer1.setCount(gameTotalPlayer1.getCount() + 1);
		...
		//if player 1 finish but player 2 not
		else {
			
			gameTotalPlayer1.setStatus("waiting for player 2 to finish");

		}
		
		result.add(gameTurn);
		result.add(gameTotalPlayer1);
		return result;
